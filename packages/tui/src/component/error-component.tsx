        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "Report copied — paste it into a support request."
                : "Copy the report and contact support to help us fix this."}
            </text>
            <text fg={colors.muted}>
              {Brand.command} {InstallationVersion}
            </text>
          </box>
        </Show>
      </box>
...
function buildIssueURL(message: string, stack: string) {
  const url = new URL(Brand.issueURL)
  url.searchParams.set("title", `TUI crash: ${message}`)
  url.searchParams.set(`${Brand.command}-version`, InstallationVersion)
  url.searchParams.set("os", describeOS())
  url.searchParams.set("terminal", describeTerminal())
  url.searchParams.set(
    "reproduce",
    `Reported automatically from the ${Brand.command} crash screen. If you can, describe what you were doing when it crashed.`,
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The ${Brand.product} TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
