...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      ...
      directories.push(path.join(current, Brand.legacyProjectDirectory))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.active = normalizeThemeName(typeof active === "string" ? active : Brand.name)
        draft.ready = false
      }),
    )
    ...
    function normalizeThemeName(theme: string) {
      return theme === Brand.legacyCommand ? Brand.name : theme
    }
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", Brand.name))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", Brand.name)
            return
          }
