...
self.onmessage = (event: MessageEvent<MarkdownWorkerRequest>) => {
  if (event.data.type === "init") {
    themeName = event.data.theme.name
...
async function highlight(request: Extract<MarkdownWorkerRequest, { type: "highlight" }>) {
  try {
    ...
    if (request.complete) {
      const result = instance.codeToTokens(request.text, { lang: language as BundledLanguage, theme: themeName })
    ...
    const stream = reset
      ? {
          language,
          source: "",
          tokenizer: new ShikiStreamTokenizer({ highlighter: instance, lang: language, theme: themeName }),
        }
      : previous
