...
const statsCanonicalBaseUrl = "https://stats.entrox.996icu.wiki/"
const statsUnfurlPath = "banner.png"
const statsUnfurlAlt = "Entrox Data wordmark on a dark patterned background"
...
export default function StatsModel() {
  ...
  const modelTitle = createMemo(() => `${modelName()} Usage, Cost & Rank | Entrox Data`)
  const modelDescription = createMemo(
    () =>
      `View ${modelName()} Entrox Go usage data, including token volume, weekly rank, token mix, costs, cache ratio, sessions, geo breakdowns, and peer models.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. Entrox Go usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                {data().rank === null
                  ? "Unranked across last week's Entrox Go usage"
                  : `Ranked #${data().rank} across last week's Entrox Go usage`}{" "}
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No Entrox Go usage in the current data window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      <SectionTitle title="Overview" description="Recent Entrox Go tokens, unique users, and market position." />
      <Show
        when={props.data}
        fallback={
          <ModelEmptyState title="No usage summary" description="This model has no Entrox Go usage rows yet." />
        }
      >
        {(data) => (
...
function ModelUsageSection(props: { data: ModelUsagePoint[] }) {
  return (
    <section id="usage" data-section="model-panel">
      <SectionTitle title="Usage" description="Daily Entrox Go token volume over the recent two-month window." />
      <Show
...
function ModelUsersSection(props: { data: ModelUsagePoint[] }) {
  return (
    <section id="users" data-section="model-panel">
      <SectionTitle
        title="Unique Users"
        description="Daily unique Entrox Go users over the recent two-month window."
      />
      <Show
...
function ModelEfficiencySection(props: { data: StatsModelData | null; catalog: ModelCatalogEntry | null }) {
  return (
    <section id="efficiency" data-section="model-panel">
      <SectionTitle title="Efficiency" description="Cost, cache behavior, and average Entrox Go session shape." />
      <Show
        when={props.data}
        fallback={
...
function ModelGeoBreakdownSection(props: { data: Record<UsageRange, CountryEntry[]> }) {
  ...
  return (
    ...
    >
      <SectionTitle title="Geo Breakdown" description="Entrox Go model tokens used by country." />
      <Show
        when={data().length > 0}
        fallback={
          <ModelEmptyState title="No geo data" description="No Entrox Go geo_stat rows matched this model." />
        }
      >
...
function ModelPeersSection(props: { data: StatsModelData | null }) {
  return (
    <section id="peers" data-section="model-panel">
      <SectionTitle title="Peers" description="Nearby models by recent Entrox Go token volume." />
      <Show
