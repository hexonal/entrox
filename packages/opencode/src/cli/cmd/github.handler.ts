...
import { extractResponseText, formatPromptTooLargeError, githubWorkflowCommentCondition } from "./github.shared"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        async function getInstallation() {
          return await fetch(`${Brand.apiURL}/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = Brand.shareBaseURL
    ...
    try {
      ...
      console.log(`${Brand.command} session`, session.id)
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || `${Brand.githubCommentMentions},/oc`)
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log(`Sending message to ${Brand.command}...`)
