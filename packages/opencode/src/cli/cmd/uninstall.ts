...
export const UninstallCommand = {
  command: "uninstall",
  describe: `uninstall ${Brand.display} and remove all related files`,
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro(`Uninstall ${Brand.display}`)
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: `npm uninstall -g ${Brand.npmPackageName}`,
      pnpm: `pnpm uninstall -g ${Brand.npmPackageName}`,
      bun: `bun remove -g ${Brand.npmPackageName}`,
      yarn: `yarn global remove ${Brand.npmPackageName}`,
      brew: `brew uninstall ${Brand.packageName}`,
      choco: `choco uninstall ${Brand.packageName}`,
      scoop: `scoop uninstall ${Brand.packageName}`,
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", Brand.packageName, "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(Brand.projectDirectory) || binDir.includes(Brand.legacyProjectDirectory)) {
  ...
  UI.empty()
  prompts.log.success(`Thank you for using ${Brand.display}!`)
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      content.includes(`# ${Brand.command}`) ||
      content.includes("# opencode") ||
      content.includes(`${Brand.projectDirectory}/bin`) ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === `# ${Brand.command}` || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (
        trimmed.includes(`${Brand.projectDirectory}/bin`) ||
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        (trimmed.includes(`${Brand.projectDirectory}/bin`) || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") &&
        (trimmed.includes(Brand.projectDirectory) || trimmed.includes(".opencode")))
    ) {
      continue
    }
