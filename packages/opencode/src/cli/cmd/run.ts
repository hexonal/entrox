...
// CLI entry point for `entrox run` and `entrox --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`entrox --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`entrox --mini --attach`): connects to a running
//      entrox server and runs interactive mode against it.
//
