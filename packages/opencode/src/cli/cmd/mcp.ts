...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info(`Remote MCP servers support OAuth by default. Add a remote server in ${Brand.configBase}.json:`)
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  const candidates = [
    path.join(baseDir, `${Brand.configBase}.json`),
    path.join(baseDir, `${Brand.configBase}.jsonc`),
    path.join(baseDir, `${Brand.legacyConfigBase}.json`),
    path.join(baseDir, `${Brand.legacyConfigBase}.jsonc`),
  ]

  if (!global) {
    candidates.push(
      path.join(baseDir, Brand.projectDirectory, `${Brand.configBase}.json`),
      path.join(baseDir, Brand.projectDirectory, `${Brand.configBase}.jsonc`),
      path.join(baseDir, Brand.legacyProjectDirectory, `${Brand.legacyConfigBase}.json`),
      path.join(baseDir, Brand.legacyProjectDirectory, `${Brand.legacyConfigBase}.jsonc`),
    )
  }
