    ...
    const result: Interface = {
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "bun", command: () => text(["bun", "pm", "ls", "-g"]) },
          {
            ...
            command: () =>
              Effect.gen(function* () {
                return [
                  yield* text(["brew", "list", "--formula", Brand.packageName]),
                  yield* text(["brew", "list", "--formula", Brand.legacyPackageName]),
                ].join("\n")
              }),
          },
          {
            ...
            command: () =>
              Effect.gen(function* () {
                return [
                  yield* text(["scoop", "list", Brand.packageName]),
                  yield* text(["scoop", "list", Brand.legacyPackageName]),
                ].join("\n")
              }),
          },
          {
            ...
            command: () =>
              Effect.gen(function* () {
                return [
                  yield* text(["choco", "list", "--limit-output", Brand.packageName]),
                  yield* text(["choco", "list", "--limit-output", Brand.legacyPackageName]),
                ].join("\n")
              }),
          },
        ]
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "brew") {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get(`https://formulae.brew.sh/api/formula/${Brand.packageName}.json`).pipe(
        ...
        if (detectedMethod === "choco") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              `https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27${Brand.packageName}%27%20and%20IsLatestVersion&$select=Version`,
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get(`https://api.github.com/repos/${Brand.releaseRepository}/releases/latest`).pipe(
            HttpClientRequest.acceptJson,
          ),
        )
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "choco":
            upgradeResult = yield* run(["choco", "upgrade", Brand.packageName, `--version=${target}`, "-y"])
            break
          case "scoop":
            upgradeResult = yield* run(["scoop", "install", `${Brand.packageName}@${target}`])
            break
          default:
