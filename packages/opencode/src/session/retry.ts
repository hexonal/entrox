...
export const GO_UPSELL_URL = `${Brand.websiteURL}/go`
...
export function retryable(error: Err, provider: string) {
  ...
  if (SessionV1.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: `Subscribe to ${Brand.product} Go for reliable access to the best open-source models, starting at $5/month.`,
          label: "subscribe",
          link: GO_UPSELL_URL,
        },
      }
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = `${Brand.websiteURL}/workspace/${workspace}/go`
      return {
