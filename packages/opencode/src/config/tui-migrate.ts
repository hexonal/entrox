...
import { DiffStyle, ScrollAcceleration, ScrollSpeed } from "@opencode-ai/tui/config"
...
const TUI_SCHEMA_URL = `${Brand.websiteURL}/tui.json`
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from legacy config files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, Brand.configBase),
    ...ConfigPaths.fileInDirectory(Global.Path.config, Brand.legacyConfigBase),
  ...
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, Brand.configBase))
    files.push(...ConfigPaths.fileInDirectory(dir, Brand.legacyConfigBase))
  }
  if (Flag.ENTROX_CONFIG) files.push(Flag.ENTROX_CONFIG)
