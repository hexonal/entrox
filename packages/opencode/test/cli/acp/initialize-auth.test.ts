...
describe("entrox acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Entrox")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.authMethods?.[0]?.id).toBe("entrox-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "entrox-login" })).toMatchObject({
          result: {},
        })
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("entrox-login")
