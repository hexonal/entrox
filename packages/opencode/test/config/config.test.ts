...
const schemaConfig = (config: object) => ({ $schema: Brand.configSchemaURL, ...config })
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, `${Brand.configBase}.jsonc`))
      expect(content).toContain(`"$schema": "${Brand.configSchemaURL}"`)
...
it.effect("updates global jsonc config when deleting a missing nested key", () =>
  withGlobalConfig({ config: { model: "test/model" }, name: "opencode.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const file = path.join(dir, `${Brand.legacyConfigBase}.jsonc`)
      const writtenConfig = yield* FSUtil.use.readFileString(file)
      const parsed = ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(writtenConfig, file), file)
